...
int inv_easy_device_init(inv_easy_device_settings_t *psettings, uint8_t *pwhoami, inv_fw_version_t * pfw_version)
{
	...
	inv_device_icm30631_init(&psettings->icm30xxx, psettings->pserif, &psettings->sensor_listener,
	     psettings->buffer, psettings->buffer_size, true /* enable device debug interface (SWD and JTAG) */);		 
#elif defined(USE_ICM30670)
	inv_device_icm30670_init(&psettings->icm30xxx, psettings->pserif, &psettings->sensor_listener,
	     psettings->buffer, psettings->buffer_size, true /* enable device debug interface (SWD and JTAG) */);
#endif
	...
	// configure mounting matrix for accelerometer and gyroscope
	...
	// configure mouting matrix for magnetometer to be aligned with the accelerometer/gyroscope from the ICM-30630
	// this is mandatory to run fusion algorithms based on both magnetometer and gyroscope/accelerometer (INV_SENSOR_TYPE_GEOMAG_ROTATION_VECTOR or INV_SENSOR_TYPE_ROTATION_VECTOR)
