...
{ 
    ...
    error.x.integral += error.x.prop * (timer.getDt()); 
    error.y.integral += error.y.prop * (timer.getDt()); 
    error.z.integral += error.z.prop * (timer.getDt()); 
...
int Xbee::get_xbee_helper(Angles& joystick_des_angles, uint8_t& joystick_thrust, uint8_t& flight_mode)
{
  ...
  //No data ready to read 
	...
	else if(num_fds ==1)
	{
		...
		int checksum_calc = checksum(data_received, DATASIZE);
		
		
		bool check1_correct =  (  data_received[first_check_index] == XBEE_START_BYTE);
 		bool check2_correct =  (  data_received[last_check_index]  == XBEE_LAST_BYTE);
 		//bool checksum_correct = (checksum_calc > 0); 	

		//printf("result: %i, check_front: %i, check_back: %i, checksum_calc: %i freq: %f \n\n", result, check1_correct, check2_correct, checksum_calc, 1/timer.getDt());
		//print_raw_bytes(data_received, 9);
		if(result == 0)
		{
			printf("read 0 bytes \n");
			//clean();
			//if( (1/timer.getDt()) < 25.00 ) tcflush(port, TCIFLUSH);
			returnVal = -3;
		}

		if( checksum_calc &&  check1_correct &&  check2_correct)
		{
		 ...
		 else
		{
			...
			else if(! check1_correct) 	      returnVal = -5;
			else if(! check2_correct)             returnVal = -6;
			else 				      returnVal = -1;
		}
	}
...
int Xbee::checksum(const uint8_t arr[], int arr_length){
	...
	{	
		//printf(" %i", (uint8_t) arr[i]);
		checksum_calc += (uint8_t) arr[i];
	}
         //combine the split 16 bit integer (two bytes: last two slots in array)
         
	
	uint16_t checksum_recv =  ( (uint8_t) arr[arr_length-2] << 8) | ( (uint8_t) arr[arr_length-1]);
...
int Xbee::check_start_thrust(void)
{
	...
	while(i<250)
	{
		...
		if(result > 0)
		{
			...
			{
				if(timerCheck.timeSinceStart() > 3.00)
				{
		...
		if(timerCheck.timeSinceStart() > 3)
                {
			printf("!!!!!NOT RECEIVING ANY JOYSTICK DATA!!!!\n");
			usleep(1000000*3);
			return -2;
...
float Xbee::timeSinceStart(void)
{
		return timer.timeSinceStart();
}
...
void Xbee::printData(Angles& desired_angles, uint8_t& joystick_thrust, uint8_t& flight_mode)
{
		printf("Last read (sec)? %f  freq: %f \n", timer.timeSinceStart(), 1/timer.getDt());
...
int main(void)
{
		...
		while(1)
		{
			...
			//int DATASIZE = 9;
			//uint8_t  data_received[DATASIZE];
			//int result = read(xbee.get_port(), &data_received[0], DATASIZE);
			//xbee.print_raw_bytes(data_received, DATASIZE);
			//tcflush(xbee.get_port(), TCIFLUSH);

			int result = 0;
			result = xbee.get_xbee_data(joystick_des_angles, joystick_thrust, flight_mode);
			//printf("result: %i \n", result);
			//xbee.printStats();
			if(result > 0) xbee.printData(joystick_des_angles, joystick_thrust, flight_mode); 
