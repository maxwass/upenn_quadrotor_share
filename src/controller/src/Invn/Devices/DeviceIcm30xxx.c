...
static void build_fifo_protocol_id_map_icm30630(inv_device_icm30xxx_t * self);
static void build_fifo_protocol_id_map_icm30631(inv_device_icm30xxx_t * self);
...
static inv_bool_t build_data_icm30630(uint8_t sensorid, uint8_t accuracy,
		const uint8_t * data, uint16_t len, inv_sensor_event_t * event);
static inv_bool_t build_data_icm30631(uint8_t sensorid, uint8_t accuracy,
		const uint8_t * data, uint16_t len, inv_sensor_event_t * event);
...
{
	assert(self && listener && serif && pvt);
		...
		uint8_t * buffer, uint32_t buffer_size, inv_bool_t debug, int chip_variant)
{
	...
	icm30xxx_serif.max_read  = inv_host_serif_get_max_read_transaction_size(self->base.serif);
	icm30xxx_serif.max_write = inv_host_serif_get_max_write_transaction_size(self->base.serif);
	icm30xxx_serif.is_spi    = inv_host_serif_is_spi(self->base.serif);
...
void inv_device_icm30630_init(inv_device_icm30xxx_t * self,
		const inv_host_serif_t * serif, const inv_sensor_listener_t * listener,
		uint8_t * buffer, uint32_t buffer_size, inv_bool_t jtag_en)
{
	device_base_init(self, serif, listener, &device_icm30xxx_vt);
	device_icm30xxx_init(self, buffer, buffer_size, jtag_en,
			INV_ICM30XXX_VARIANT_ICM30630);

	build_fifo_protocol_id_map_icm30630(self);

	self->build_data = build_data_icm30630;
}

void inv_device_icm30631_init(inv_device_icm30xxx_t * self,
		const inv_host_serif_t * serif, const inv_sensor_listener_t * listener,
		uint8_t * buffer, uint32_t buffer_size, inv_bool_t jtag_en)
{
	device_base_init(self, serif, listener, &device_icm30xxx_vt);
	device_icm30xxx_init(self, buffer, buffer_size, jtag_en,
			INV_ICM30XXX_VARIANT_ICM30631);

	build_fifo_protocol_id_map_icm30631(self);

	self->build_data = build_data_icm30631;
	...
	inv_fifo_protocol_data_packet_number_set_r(FIFOPROTOCOL_SENSOR_ID_EXT_10,
			2, self->icm30xxx_states.fifop_exp_pkt_cnt);
	/* Set data packet number for Energy Expanditure */
...
void inv_device_berlin_init(inv_device_icm30xxx_t * self,
		...
		uint8_t * buffer, uint32_t buffer_size, inv_bool_t jtag_en)
{
	device_base_init(self, serif, listener, &device_berlin_vt);
	device_icm30xxx_init(self, buffer, buffer_size, jtag_en,
...
static int poll_device_common(inv_device_icm30xxx_t * self, uint8_t * fifo_mask)
{
	...
	if((rc = inv_icm30xxx_read_int_status(&self->icm30xxx_states,
			self->icm30xxx_states.poll_int_mask, fifo_mask)) != 0)
		return rc;

	if((rc = inv_icm30xxx_ctrl_poll_sensor(&self->icm30xxx_states, *fifo_mask)) != 0)
		return rc;
...
int inv_device_icm30xxx_enable_sensor(void * context, int sensor, inv_bool_t en)
{
	...
	if(self->icm30xxx_states.variant == INV_ICM30XXX_VARIANT_ICM30670
		&& INV_SENSOR_ID_TO_TYPE(sensor) == INV_SENSOR_TYPE_TSIMU) {
		...
		const void * value, unsigned size)
{
	...
	if(self->icm30xxx_states.variant == INV_ICM30XXX_VARIANT_ICM30670) {
		...
		return inv_icm30xxx_cmd_set_sensor_config(&self->icm30xxx_states, sid,
				setting-INV_SENSOR_CONFIG_CUSTOM, value, size);
	}
	else {
		return INV_ERROR_NIMPL;
	}
...
static void build_fifo_protocol_id_map_icm30631(inv_device_icm30xxx_t * self)
{
	...
	self->map_2sensortype[FIFOPROTOCOL_SENSOR_ID_EXT_11]  = INV_SENSOR_TYPE_ENERGY_EXPANDITURE;
		...
		const uint8_t * data, uint16_t len, inv_sensor_event_t * event)
{
	...
	case FIFOPROTOCOL_SENSOR_ID_EXT_10: // FLOOR_CLIMB_COUNTER
		...
		break;
	case FIFOPROTOCOL_SENSOR_ID_EXT_11: // ENERGY_EXPANDITURE
		...
		const uint8_t * data, uint16_t len, inv_sensor_event_t * event)
{
	...
	case FIFOPROTOCOL_SENSOR_ID_HEART_RATE:
		inv_fifo_protocol_helper_decode_heart_rate_data(data, len,
			&event->data.hrm.bpm);
		if(self->icm30xxx_states.variant == INV_ICM30XXX_VARIANT_ICM30631) {
