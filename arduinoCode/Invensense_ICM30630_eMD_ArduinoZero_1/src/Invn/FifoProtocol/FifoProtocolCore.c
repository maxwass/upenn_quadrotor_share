...
static const uint8_t sDataPacketNumber[FIFOPROTOCOL_SENSOR_ID_MAX] = 
{
	...
	/* FIFOPROTOCOL_SENSOR_ID_GYROMETER 				*/ 1,
	...
	/* FIFOPROTOCOL_SENSOR_ID_UNCAL_GYROMETER 			*/ 2,
...
static uint8_t sDataPacketNumberExt[FIFOPROTOCOL_SENSOR_ID_EXT_CNT];
...
void inv_fifo_protocol_data_packet_number_set(uint8_t sensorid, uint8_t number)
{
	...
	ASSERT(sensorid >= FIFOPROTOCOL_SENSOR_ID_EXT_BASE &&
			sensorid < FIFOPROTOCOL_SENSOR_ID_EXT_MAX);
	ASSERT(number > 0 && number <= FIFOPROTOCOL_NUMBER_MAX);

	sDataPacketNumberExt[sensorid - FIFOPROTOCOL_SENSOR_ID_EXT_BASE] = number;
}

static uint8_t get_expected_packet_number_data(uint8_t sensorid)
{
	...
	else if(sensorid < FIFOPROTOCOL_SENSOR_ID_EXT_MAX) {
		packet_number = sDataPacketNumberExt[sensorid - FIFOPROTOCOL_SENSOR_ID_EXT_BASE];
	}
		...
		*/
		if(packet->type == FIFOPROTOCOL_PACKET_TYPE_COMMAND) {
			...
			if(answer) {
				...
				packet->states.expected_packet_number = get_expected_packet_number_data(sensorid);
			}
		}
...
)
{
	...
	if(packet->states.packet_cnt == 0) {
		if(packet->type == FIFOPROTOCOL_PACKET_TYPE_DATA) {
			raw_packet[2] |= packet->u.data.accuracy << FIFOPROTOCOL_ACCURACY_SHIFT;
			raw_packet[2] |= packet->u.data.status << FIFOPROTOCOL_STATUS_SHIFT;
			...
			packet->states.expected_packet_number = get_expected_packet_number_data(packet->sensorid);
		} else {
