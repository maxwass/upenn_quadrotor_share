...
int inv_icm30xxx_soft_reset(struct inv_icm30xxx * s)
{
	...
	inv_icm30xxx_sleep(10); /* likely need a sleep after reset...
	                  not sure how much is appropriate */

	/* now wake up Icm30xxx */
	return inv_icm30xxx_wake_up(s);
}
		...
		uint8_t * buf, uint32_t len)
{
	...
	inv_int32_to_big8(addr, mem_addr_buf);
		...
		uint32_t source_addr, uint32_t dest_addr, uint32_t len)
{
	...
	inv_int32_to_little8(source_addr,dma_addr_buf);	   /* put source address LSB first */
	inv_int32_to_little8(dest_addr, &dma_addr_buf[4]); /* append dest address LSB first */
	...
	inv_int32_to_little8(len, dma_len_buf);
		...
		uint32_t flash_size)
{
	...
	const unsigned ICM30XXX_FLASH_SIZE = ((s->variant) == INV_ICM30XXX_VARIANT_ICM30630) ?
			ICM30630_FLASH_SIZE : ICM30670_FLASH_SIZE;

	const unsigned ICM30XXX_SRAM_SIZE = ((s->variant) == INV_ICM30XXX_VARIANT_ICM30630) ?
			ICM30630_SRAM_SIZE : ICM30670_SRAM_SIZE;

	const unsigned ICM30XXX_SRAM_START_ADDR = ((s->variant) == INV_ICM30XXX_VARIANT_ICM30630) ?
			ICM30630_SRAM_START_ADDR : ICM30670_SRAM_START_ADDR;

	const unsigned ICM30XXX_FLASH_START_ADDR = ((s->variant) == INV_ICM30XXX_VARIANT_ICM30630) ?
		ICM30630_FLASH_START_ADDR : ICM30670_FLASH_START_ADDR;

	INV_MSG(INV_MSG_LEVEL_DEBUG, "Inhibiting LP_EN...");
	...
	INV_MSG(INV_MSG_LEVEL_DEBUG, "Waking up Icm30xxx...");

	/* make sure Icm30xxx is awake before writing memory */
	...
	INV_MSG(INV_MSG_LEVEL_DEBUG, "Soft reseting Icm30xxx...");
	...
	INV_MSG(INV_MSG_LEVEL_DEBUG, "Iniating DMA transfer of %lu bytes from 0x%x to 0x%x...",
			flash_size, ICM30XXX_SRAM_SIZE, ICM30XXX_FLASH_SIZE);
	...
	INV_MSG(INV_MSG_LEVEL_DEBUG, "Restoring LP_EN...");
...
int inv_icm30xxx_get_cpu_ticks(struct inv_icm30xxx * s, uint32_t * ticks)
{
	...
	*ticks = inv_little8_to_int32(buffer);
