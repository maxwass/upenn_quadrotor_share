...
typedef struct inv_host_serif
{
	...
	/** @brief Maximum number of bytes allowed per serial transaction
	 */
...
/** @brief Helper method to get max transaction size value of a Serial Interface object
 */
static inline uint32_t inv_host_serif_get_max_transaction_size(const inv_host_serif_t * instance)
{
	assert(instance);

	return instance->max_transaction_size;
}
...
static inline uint32_t inv_host_serif_get_max_transaction_size_i(void)
{
	return inv_host_serif_get_max_transaction_size(inv_host_serif_get_instance());
}
