...
typedef struct 
{
	...
	const float            acc_gyr_mounting_matrix[9];                    /*!< mounting matrix for accelerometer and gyroscope */
	const float            mag_mounting_matrix[9];                        /*!< mounting matrix for magnetometer */
} inv_easy_device_settings_t;


/** 
 * Sensors event structure for timestamp statistics
 */
typedef struct {
	unsigned int count;
