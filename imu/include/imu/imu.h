...
class Imu {
 ...
 public:
  ...
  int calibrate(void)
  {
    ...
    while(calibrateTimer.timeSinceStart() < 10)
      {
	...
	if(suc_read<0);// printf("printing old values\n \n \n \n \n \n");
	else if(suc_read == 1)
	  {
	    ...
	    bias.phi       += cal_data.phi;
	    bias.theta     += cal_data.theta;
    ...
    printf("bias (averaged) :  theta: %5.3f phi: %5.3f theta_dot: %5.3f  phi_dot: %5.3f  psi_dot: %5.3f psi_magn: %5.3f  iterations: %i  \n\n", bias.theta/i, bias.phi/i, bias.theta_dot/i, bias.phi_dot/i, bias.psi_dot/i,bias.psi_magn_continuous/i, i);
    
    bias.theta        = bias.theta/i;
    bias.phi  	      = bias.phi/i;
