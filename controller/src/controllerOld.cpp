...
Sonar sonar_y_neg("/dev/ttyUSB1", minDist, maxDist);
...
PIDController altitudeObj(gains.kp_altitude, gains.kd_altitude, gains.ki_altitude);
...
*/
	...
	if(new_vo){
		...
		vel_x *= (altitudeObj.getValue() + 0.05)/1.5;
	        vel_y *= (altitudeObj.getValue() + 0.05)/1.5;
		
		//printf("vel_x *= (altitudeObj.getValue() + 0.05)/1.5: %f, altitudeObj.getValue(): %f \n", vel_x, altitudeObj.getValue());
		//printf("vel_y *= (altitudeObj.getValue() + 0.05)/1.5: %f, altitudeObj.getValue(): %f \n", vel_y, altitudeObj.getValue()); 
...
/*
	incLimImu++;	
	if(incLimImu > iterUntilImuRead
	{
		incLimImu = 0;
		timeLimitImu = 0.00;
		new_imu_data = imu.get_imu_calibrated_data(imu_data);
	}
	*/
	
		//new_imu_data = imu.get_imu_calibrated_data_ns(imu_data);
...
State error_imu(const State& imu_data, const int new_imu_data, const Angles& desired_angles){
	...
	//printf("Outside: new)imu_data: %i, imu_data.altitude_calibrated: %f \n",new_imu_data, imu_data.altitude_calibrated);
	altitudeObj.update(imu_data.altitude_calibrated, imu_data.dt);
     }



    error.altitude_calibrated     =     altitudeObj.getPropError();
    error.altitude_deriv 	  =     altitudeObj.getDerivError();
...
Control_command thrust(const State& imu_error, const State_Error& vicon_error, const Control_command& U_trim, const int joystick_thrust, const Gains& gains){
    ...
    {
        U.thrust = nominal_thrust + gains.kp_altitude*altitudeObj.getPropError() + gains.kd_altitude*altitudeObj.getDerivError() + gains.ki_altitude*altitudeObj.getDerivError();
...
void log_data(const Distances& sonar_distances, const float& dt, const State_Error& vicon_error, const State& imu_data, const State& imu_error, const Angles& , const int thrust){
...
void display_info(const Distances& sonar_distances, const int succ_read,  const State& imu_data,  const State& imu_error, const State_Error& vicon_error, const Control_command& U, const Angles& desired_angles,const int  joystick_thrust, const int  flight_mode, const Times& times, const Times& time_m){
	...
	printf("altitudeObj:   %7.2f, altitude imu:  %7.2f, desired_altitude:  %7.2f  max: %7.2f, min: %7.2f,  altitude_deriv:   %7.2f,  prop_error:   %7.2f,  deriv_error:   %7.6f, integral_error:   %7.2f\n\n", altitudeObj.getValue(),imu_data.altitude_calibrated, altitudeObj.getDesiredValue(),altitudeObj.getMaxValue(),  altitudeObj.getMinValue(), altitudeObj.getValueDeriv(), altitudeObj.getPropError(), altitudeObj.getDerivError(), altitudeObj.getIntegralError());
...
// control of desired positions
    ...
    //START GAINS         
	      ...
	      printf("increase d gains vision : %f \n " , gains.kd_x);
                break;
